"""
Homework assignment:
    This code does not work in its current state. Your task is to figure out why and make such changes that it 
    will work.
    
    Requirements:
        - The only thing you can change in main() method is the series_databases list.
        - You cannot change 'print_databases_rating' function.
        - You have to load IMDB data only through 'imdb.database.IMDBDatabase' class (you cannot load it by yourself
            from file.
        - You cannot change anything in 'imdb' package!
        - Other than that, you are free to add any number of needed classes/functions.
"""
import json
